...
const Popup = ({record, pin_check,un_pin_check,delete_check,get_assigned_invoices,open_edit_check_modal,set_check_paid,set_check_unpaid,open_check_description,visible, x, y}) => visible &&
  <ul className="popup"  id='popupMenu' style={{left: `${x}px`, top: `${y}px`}}>
    <li Style={record.is_for_sup =='Supplier' ? 'display:block':'display:none' } onClick={() =>get_assigned_invoices(record)}> <LaunchIcon /> Voir les factures</li>
    <li Style={record.is_for_sup =='Expense' ? 'display:block':'display:none' } onClick={() =>open_check_description(record)}> <LaunchIcon /> Voir description</li>
